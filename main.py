...
class Pendulum:
    ...
    def __init__(self, color, window, height=100, width=10, gravity=True, mass=5, angle=0, omega=0):
        self.color = color
        self.height = height
        self.width = width
        self.window = window
    ...
    def draw(self):
        ...
        end_pos_x = self.x - self.height*np.cos(self.angle*2*np.pi)
        end_pos_y = self.y - self.height * np.sin(self.angle * 2 * np.pi)
        pygame.draw.line(self.window, self.color, (self.x, self.y),
                         (end_pos_x, end_pos_y), self.width)
